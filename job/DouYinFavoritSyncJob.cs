...
namespace dy.net.job
{
    public class DouyinFavoritSyncJob : DouyinBasicSyncJob
    {
        ...
        {
            return await douyinCookieService.GetOpendCookiesAsync(x => !string.IsNullOrWhiteSpace(x.SecUserId));
        }
        ...
        protected override string CreateSaveFolder(DouyinCookie cookie, Aweme item, AppConfig config, DouyinFollowed followed, DouyinCollectCate cate)
        {
            ...
            var folder = Path.Combine(config.SavePath,cookie.UserName,VideoType.GetDesc(), authorFolder, $"{DouyinFileNameHelper.SanitizeLinuxFileName(item.Desc, item.AwemeId, true)}");
            ...
            else
            {
                ...
                folder = Path.Combine(config.SavePath, cookie.UserName, VideoType.GetDesc(), authorFolder, $"{DouyinFileNameHelper.SanitizeLinuxFileName(item.Desc, item.AwemeId, true)}" + "_" + item.AwemeId);
            }
            return folder;
        }
    }
