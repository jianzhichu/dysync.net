...
namespace dy.net.job
{
    public class DouyinCollectCustomSyncJob : DouyinBasicSyncJob
    {
        ...
        protected override string CreateSaveFolder(DouyinCookie cookie, Aweme item, AppConfig config, DouyinFollowed followed, DouyinCollectCate cate)
        {
            if (cate != null)
            {
                var folder = Path.Combine(config.SavePath, cookie.UserName, VideoType.GetDesc(), DouyinFileNameHelper.SanitizeLinuxFileName(cate.SaveFolder, "", true), DouyinFileNameHelper.SanitizeLinuxFileName(item.Desc, item.AwemeId, true));
                ...
                else
                {
                    ...
                    folder = Path.Combine(config.SavePath, cookie.UserName, VideoType.GetDesc(), DouyinFileNameHelper.SanitizeLinuxFileName(cate.SaveFolder, "", true), DouyinFileNameHelper.SanitizeLinuxFileName(item.Desc, item.AwemeId, true)+"_"+item.AwemeId);
                }
                return folder;
            }
            else
